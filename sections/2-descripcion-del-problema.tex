...
\subsection{Proposición de intervención}

A partir del análisis anterior, este trabajo plantea la siguiente proposición central, que orienta sus objetivos de intervención:

\begin{quote}
  \textbf{Una estrategia de \textit{evolución arquitectónica híbrida} sobre la plataforma Paralegales permite mitigar los riesgos de seguridad, la fragmentación arquitectónica y los déficits de mantenibilidad del sistema, sin incurrir en el costo operacional, el riesgo de interrupción del servicio ni la disrupción organizacional que implicaría una migración total del proveedor de nube. Dicha estrategia se materializa en cuatro líneas de intervención complementarias: (i) la reubicación sistemática de la lógica de negocio crítica desde el cliente hacia entornos de ejecución del lado del servidor —mediante funciones \textit{serverless} para la lógica de servicios y un \textit{Backend-for-Frontend} para los flujos cercanos a la sesión del usuario—; (ii) una organización modular del \textit{frontend} bajo principios de \textit{Domain-Driven Design}; (iii) una capa de servicios \textit{backend} con separación estricta \textit{Handler--Service--Repository}; y (iv) capacidades transversales de observabilidad estructurada y aseguramiento de calidad continuo.}
\end{quote}

\subsubsection*{Supuestos subyacentes}

La proposición anterior descansa sobre un conjunto de supuestos que conviene explicitar, dado que delimitan su ámbito de validez:
...
La proposición articula su efecto esperado sobre cinco dimensiones del sistema, cada una de las cuales se conecta con uno o varios de los frentes problemáticos descritos previamente:
...
Estas dimensiones constituyen el marco bajo el cual se valoran las decisiones técnicas adoptadas a lo largo del proyecto y, a la vez, sirven de referencia implícita para juzgar el grado de cumplimiento de la proposición al cierre del trabajo.
