...
La configuración de Nuxt integra \textit{auto-imports} para reducir la verbosidad en el desarrollo: \textit{composables}, utilidades y \textit{stores} de \texttt{common/} están disponibles globalmente sin importaciones explícitas. El estado global se centraliza en \textit{stores} de Pinia alojadas en \texttt{common/stores/}, permitiendo que \textit{features} independientes compartan información de forma controlada sin acoplamiento directo entre ellas. Esta arquitectura está orientada a beneficios concretos de mantenibilidad: que nuevas \textit{features} se incorporen sin riesgo de conflicto con el código existente, que los cambios en un dominio no requieran análisis de impacto sobre el resto del proyecto, y que el aislamiento del tema permita actualizar la biblioteca de UI sin comprometer la lógica de negocio. La evidencia del resultado alcanzado por esta refactorización se presenta en el \autoref{subsec:resultado-b}.
...
\begin{enumerate}
  ...
  \item \textbf{Abstracción del repositorio (\texttt{repository\_interface.go} y \texttt{repository\_firestore.go}):} define una interfaz genérica que especifica las operaciones de persistencia (lectura, escritura y consultas). Una implementación concreta realiza las operaciones reales contra Firestore. Esta abstracción constituye el núcleo de la flexibilidad arquitectónica del componente.
\end{enumerate}
